...
def harmonize(df):
    ...
    norm_lookup = {norm(c): c for c in cols}

    DATE_ALIASES = [
        "crashdate","crash_date","crash date","date",
        "collisiondate","accidentdate","incidentdate"
    ]
    TIME_ALIASES = [
        "crashtime","crash_time","crash time","time",
        "collisiontime","accidenttime","incidenttime"
    ]
    LAT_ALIASES  = ["latitude","lat","y","ycoord","ycoordinate"]
    LON_ALIASES  = ["longitude","lon","lng","long","x","xcoord","xcoordinate"]
    BORO_ALIASES = ["borough","boro","county","neighborhood"]

    def find_alias(aliases):
        for a in aliases:
            if a in norm_lookup: return norm_lookup[a]
        # try partial contains (e.g., "date" in "crashdate")
        for c in cols:
            if any(a in norm(c) for a in aliases):
                return c
        return None

    cdate = find_alias(DATE_ALIASES)
    ctime = find_alias(TIME_ALIASES)
    clat  = find_alias(LAT_ALIASES)
    clon  = find_alias(LON_ALIASES)
    cbor  = find_alias(BORO_ALIASES)

    out = df.copy()

    if not cdate:
        # last resort: look for any column that parses as a date strongly
        for c in cols:
            sample = pd.to_datetime(out[c], errors="coerce")
            if sample.notna().mean() > 0.6:  # 60%+ parseable as dates
                cdate = c; break
    if not cdate:
        raise KeyError("No crash date column found. Looked for aliases like 'CRASH DATE', 'CRASH_DATE', 'collision date', etc.")
    ...
    if ctime and ctime in out:
        ...
        # If no time column, default to date at midnight
    ...
    # NYC-ish bbox guard (only applied when coords exist)
