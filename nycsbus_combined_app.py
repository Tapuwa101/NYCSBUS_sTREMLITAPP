    ...
    if not len(crashes):
        ...
        if not seasons:
            ...
            if "crash_datetime" in df_season.columns and not df_season.empty:
                ...
                heat = (df_season.groupby(["dow","hour"]).size().unstack("hour")).reindex(dow_order)
                ...
                ax.set_title(f"{sel}: DOW × Hour heatmap")
                cbar = fig.colorbar(im, ax=ax); cbar.set_label("Crashes")
                st.pyplot(fig, clear_figure=True)
            else:
            ...
            if "borough" in crashes.columns and crashes["borough"].notna().any():
                ...
                pivot["delta"] = pivot["season"] - pivot["non_season"]
                fig, ax = plt.subplots(figsize=(8, 4))
                pivot.sort_values("delta").plot(kind="barh", y=["non_season","season"], ax=ax)
                ax.set_title(f"{sel}: crashes by borough (season vs non-season)")
                st.pyplot(fig, clear_figure=True)
